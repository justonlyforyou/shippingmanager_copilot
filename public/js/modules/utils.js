...
export async function loadServerConfig() {
  try {
    ...
    // Set current config values - NO fallbacks, server must provide valid data
    if (configData.success && configData.config) {
      if (!configData.config.host || !configData.config.port) {
        console.error('[Server Config] Invalid config data - missing host or port');
        return;
      }
      hostSelect.value = configData.config.host;
      document.getElementById('serverPort').value = configData.config.port;
      updateHostInfo();
    }
  } catch (error) {
