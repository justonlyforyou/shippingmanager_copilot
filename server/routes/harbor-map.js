...
router.get('/overview', async (req, res) => {
  try {
    ...
    // Enrich ports with demand data from SQLite (for ports without demand from API)
    // This provides demand data for ALL ports, not just assigned ones
    const latestDemand = portDemandStore.getLatestDemand(userId);
    const demandByPort = new Map(latestDemand.map(d => [d.port_code, d]));

    const enrichedPorts = ports.map(port => {
      // If port already has proper demand data (object with container/tanker), keep it
      if (port.demand && typeof port.demand === 'object' && !Array.isArray(port.demand) &&
          (port.demand.container || port.demand.tanker)) {
        return port;
      }

      // Otherwise, try to get demand from SQLite
      const storedDemand = demandByPort.get(port.code);
      if (storedDemand) {
        return {
          ...port,
          demand: {
            container: {
              dry: storedDemand.dry_demand,
              refrigerated: storedDemand.refrigerated_demand
            },
            tanker: {
              fuel: storedDemand.fuel_demand,
              crude_oil: storedDemand.crude_demand
            }
          },
          consumed: {
            container: {
              dry: storedDemand.dry_consumed,
              refrigerated: storedDemand.refrigerated_consumed
            },
            tanker: {
              fuel: storedDemand.fuel_consumed,
              crude_oil: storedDemand.crude_consumed
            }
          }
        };
      }
    ...
    const portsWithDemandFromStore = enrichedPorts.filter(p => demandByPort.has(p.code)).length;
    logger.debug(`[Harbor Map] Enriched ${portsWithDemandFromStore} ports with demand from SQLite store`);
